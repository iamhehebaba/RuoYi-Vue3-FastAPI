...
def _match_rule(proxy_rules: List[ProxyRule], sub_path: str, method: str) -> Optional[ProxyRule]:
    """
...
async def _require_permission_for_path(
    ...
    rule = _match_rule(proxy_rules, sub_path, method)
    if not rule:
        # 未配置的路径，不允许访问
...
async def proxy_rule_executor(
    ...
    await _require_permission_for_path(proxy_rules, request, current_user)
